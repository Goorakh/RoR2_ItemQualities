...
namespace ItemQualities
{
    static class CustomInteractableHandler
    {
        ...
        static bool containsAnySmallCategoryChest(DirectorCardCategorySelection dccs)
        {
            foreach (DirectorCardCategorySelection.Category category in dccs.categories)
            {
                foreach (DirectorCard card in category.cards)
        ...
        static bool containsAnyLargeCategoryChest(DirectorCardCategorySelection dccs)
        {
            foreach (DirectorCardCategorySelection.Category category in dccs.categories)
            {
                foreach (DirectorCard card in category.cards)
                {
                    if (matchDirectorCard(card, "iscCategoryChest2Damage", RoR2_DLC1_CategoryChest2.iscCategoryChest2Damage_asset) ||
